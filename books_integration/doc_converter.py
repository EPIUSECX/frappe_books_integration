...
# Portions adapted for Cohenix bench (company-based SI/PE, no POS requirement).
...
class DocConverterBase:
	def __init__(self, instance, dirty_doc, target: str) -> None:
		...
		self.target_doctype = get_doctype_name(self.source_doctype, self.target, self._dirty_doc)
...
class Item(DocConverterBase):
	...
	def _fill_missing_values_for_fbooks(self):
		...
		if getattr(self.settings, "sync_item_as_non_inventory", None):
	...
	def _fill_missing_values_for_erpn(self):
		...
		self.converted_doc["item_group"] = ig[0].name if ig else "All Item Groups"

		if self._dirty_doc.get("tax"):
			self.converted_doc["taxes"] = []
			self.converted_doc["taxes"].append(
				{"item_tax_template": self.get_item_tax_template(self._dirty_doc.get("tax"), self.target)}
			)

		if self.doc_dict.get("barcode"):
			self.converted_doc["barcodes"] = []
			self.converted_doc["barcodes"].append({"barcode": str(self._dirty_doc.get("barcode"))})
...
class SalesInvoice(DocConverterBase):
	...
	def _fill_missing_values_for_erpn(self):
		company = frappe.db.get_value("Books Instance", self.instance, "company")
		if not company:
			frappe.throw(_("Set Company on Books Instance {0}").format(self.instance))

		self.converted_doc["company"] = company
		self.converted_doc["is_pos"] = 0
		self.converted_doc["disable_rounded_total"] = 1
		self.converted_doc["set_posting_time"] = 1
		self.converted_doc["posting_date"] = getdate(self.converted_doc.get("posting_date") or today())

		if not self.converted_doc.get("customer"):
			frappe.throw(_("Sales Invoice is missing customer / party"))

		for item in self.converted_doc.get("items") or []:
			...
			if ic:
				item.setdefault(
					...
					frappe.db.get_value(
						"Item Default",
						{"parent": ic, "company": company},
						"income_account",
					)
					or frappe.db.get_value("Company", company, "default_income_account"),
				)
				item.setdefault("uom", frappe.db.get_value("Item", ic, "stock_uom") or "Nos")
			if flt(item.get("discount_percentage")) > 0:
				discount_amount = flt(
					(flt(item.get("price_list_rate")) * flt(item.get("discount_percentage"))) / 100
				)
				item["discount_amount"] = discount_amount
				item["rate"] = flt(item.get("price_list_rate")) - discount_amount
...
class PaymentEntry(DocConverterBase):
	...
	def _fill_missing_values_for_erpn(self):
		...
		party = self.converted_doc.get("party") or self._dirty_doc.get("party")
		if not party:
			frappe.throw(_("Payment missing party"))
		self.converted_doc["party"] = party
		...
		amt = flt(self.converted_doc.get("total_allocated_amount") or self._dirty_doc.get("amount") or 0)
		self.converted_doc["received_amount"] = amt
		self.converted_doc["paid_amount"] = amt

		mode = self.converted_doc.get("mode_of_payment") or "Cash"
		bank = None
		...
		if not bank:
			frappe.throw(_("Could not resolve Bank/Cash account for Payment Entry"))

		party_account = get_party_account(
			self.converted_doc["party_type"], self.converted_doc["party"], company
		)

		ptype = self.converted_doc.get("payment_type") or "Receive"
		if ptype == "Receive":
			self.converted_doc["paid_from"] = party_account
			self.converted_doc["paid_to"] = bank.account
		else:
			self.converted_doc["paid_from"] = bank.account
			self.converted_doc["paid_to"] = party_account

		self.converted_doc["posting_date"] = getdate(self.converted_doc.get("posting_date") or today())

		for row in self.converted_doc.get("references") or []:
			...
			row["reference_doctype"] = get_doctype_name(rdt, self.target, {"doctype": rdt})
			row["total_amount"] = float(row.get("total_amount") or 0)
			row["allocated_amount"] = float(row.get("total_amount") or 0)
...
class StockEntry(DocConverterBase):
	...
	def _fill_missing_values_for_fbooks(self):
		...
		self.converted_doc["movementType"] = "".join(self.converted_doc["movementType"].split())

		for item in self.converted_doc["items"]:
			self.converted_doc["serial_no"] = not item.get("use_serial_batch_fields") and item.get(
				"serial_and_batch_bundle"
			)

			item["serial_no"] = item.get("item_code")
			if not item.get("use_serial_batch_fields") and item.get("serial_and_batch_bundle"):
...
class ItemPrice(DocConverterBase):
	...
	def _fill_missing_values_for_fbooks(self):
		self.converted_doc["parentSchemaName"] = get_doctype_name("Price List", "fbooks")
...
class DeliveryNote(DocConverterBase):
	...
	def before_save(self):
		...
		doc_status = frappe.db.get_value("Sales Invoice", ref_doc_name_in_erpnext, "docstatus")
